...
function bind_budget_modal_buttons() {
  document
    .getElementById('createBudgetBtn')
    ?.addEventListener('click', async () => {
      populate_categories_select(document.getElementById('budgetCategory'));
...
function bind_budget_forms() {
  document
    ...
    ?.addEventListener('submit', create_budget_submit);
  document
    .getElementById('editBudgetForm')
    ?.addEventListener('submit', update_budget_submit);
}

function bind_delete_budget_button() {
  document
    .getElementById('confirmDeleteBudgetBtn')
    ?.addEventListener('click', delete_budget_submit);
}
...
function bind_budget_actions() {
  ...
  container.addEventListener('click', async (e) => {
    ...
    const budget = budgets.find(
      (b) => String(b.id || b._id) === String(budgetId),
    );
    if (!budget) return;
...
function open_edit_budget_modal(budget) {
  ...
  populate_categories_select(
    document.getElementById('editBudgetCategory'),
    budget.category.id,
  );
...
async function update_budget_submit(e) {
  ...
  try {
    await update_budget(currentEditingBudget.id, {
...
function render_budgets() {
  ...
  budgets.forEach((budget) => {
    fragment.appendChild(create_budget_card(budget));
  });
...
function create_budget_card(budget) {
  const { id, _id, name, limit, category } = budget;
